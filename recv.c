...
int print_msg(int send_fd, char* buf, char* err_message)
{
    int r_size = -1;
...
void recv_mail()
{
    ...
    print_msg(s_fd,buf,"send_username");

    sprintf(buf,"pass %s\r\n",pass);
    send(s_fd,buf,strlen(buf),0);
    printf("\033[1;32m%s\033[0m", buf);
    print_msg(s_fd,buf,"send_password");
    ...
    r_size = print_msg(s_fd,buf,"RETR");
    total_length -= r_size;       
    // total length of mail > buf's length                      
